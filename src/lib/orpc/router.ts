...
const listStuffProcedure = publicProcedure.output(listStuffsOutputSchema).handler(async () => {
  return await listStuffs()
})
...
const createStuffProcedure = authorizedProcedure
  ...
  .handler(async ({ input, context }) => {
    return await createStuff(input, context.user.id)
  })
...
const updateStuffProcedure = authorizedProcedure
  ...
  .output(stuffSchema)
  .handler(async ({ input, context }) => {
    const updatedStuff = await updateStuff(input, context.user.id)
